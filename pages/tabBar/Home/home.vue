	...
	<view>
		...
		<view class="swiper">
			<view class="swiper-box">
				...
				<view class="indicator"><view :class="['dots', { on: currentSwiper >= index }]" v-for="(swiper, index) in swiperList" :key="index"></view></view>
			</view>
		</view>
		<!-- 分类 -->
		...
		<!-- 广告图 -->
		<view v-if="promotion.length > 0" class="banner"><image src="/static/img/category/ad.jpg" mode=""></image></view>
		<!-- 活动区 -->
		...
		initData() {
			this.request({
				...
				success: res => {
					...
					let { swiperList, categoryList, promotion } = res.data;
